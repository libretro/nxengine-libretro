		...
		case 100:
		{
			...
			if (o->y > 0x10000)
			{
				...
				if (o->blockd)
				{
					...
					for(int i=0;i<4;i++)
					{
						Object *s = SmokePuff(o->x + random_nx(-12<<CSF, 12<<CSF), \
											  o->y + 0x2000);
						
						s->xinertia = random_nx(-0x155, 0x155);
						s->yinertia = random_nx(-0x600, 0);
						s->PushBehind(o);
					}
				}
			}
	...
	switch(o->state)
	{
		...
		case 1:
		{
			if (--o->timer < 0)
			{
				...
				dv->xinertia = random_nx(-16<<CSF, 16<<CSF);
...
void ai_green_devil(Object *o)
{
	switch(o->state)
	{
		case 0:
		{
			...
			o->ymark = o->y;
			o->yinertia = random_nx(-5<<CSF, 5<<CSF);
			o->state = 1;
		}
		case 1:
		{
...
void ai_bute_archer_red(Object *o)
{
	...
	switch(o->state)
	{
		case 0:
		{
			...
			o->yinertia = random_nx(-0x400, 0x400);
		}
