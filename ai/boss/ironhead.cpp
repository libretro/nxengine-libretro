...
void IronheadBoss::Run(void)
{
	...
	switch(o->state)
	{
		...
		case IRONH_SPAWN_FISHIES+1:		// wave of fishies comes in
		{
			...
			if ((o->timer & 3)==0)
			{
				CreateObject((random_nx(15, 18) * TILE_W) << CSF, \
						  	(random_nx(ARENA_TOP, ARENA_BOTTOM) * TILE_H) << CSF, \
						  	OBJ_IRONH_FISHY);
			}
		}
		break;
		...
		case IRONH_SWIM:		// swimming attack
		{
			...
			{	// returning from right side of screen
				o->x = 0x5a000;
				o->y = (random_nx(ARENA_TOP, ARENA_BOTTOM) * TILE_H) << CSF;
			}
			
			o->xmark = o->x;
			o->ymark = o->y;
			
			o->yinertia = random_nx(-0x200, 0x200);
			o->xinertia = random_nx(-0x200, 0x200);
			...
			if (o->dir==LEFT)
			{
				...
				switch(++o->timer)
				{
					...
					case 320:
					{
						...
						shot->xinertia = (random_nx(-3, 0) << CSF);
						shot->yinertia = (random_nx(-3, 3) << CSF);
						sound(SND_EM_FIRE);
					}
					break;
				}
...
Object *smoke;
	...
	smoke = CreateObject(o->CenterX() + (random_nx(-128, 128)<<CSF), \
						 o->CenterY() + (random_nx(-64, 64)<<CSF),
						 OBJ_SMOKE_CLOUD);
	
	smoke->xinertia = random_nx(-128, 128);
	smoke->yinertia = random_nx(-128, 128);
}
...
Object *brick;
	...
	{
		o->state = 1;
		o->timer = random_nx(0, 200);
	}
	
	if (!o->timer)
	{	// time to spawn a block
		o->state = 0;
		brick = CreateObject(o->x, o->y + (random_nx(-20, 20) << CSF), OBJ_IRONH_BRICK);
		brick->dir = o->dir;
	}
	else o->timer--;
}
...
void ai_ironh_brick(Object *o)
{
	if (!o->state)
	{
		...
		o->xinertia = random_nx(0x100, 0x200);
...
void ai_ikachan_spawner(Object *o)
{
	switch(o->state)
	{
		...
		case 10:	// yay spawn ikachans!
		{
			...
			if ((o->timer & 3) == 1)
			{
				CreateObject(o->x, o->y + ((random_nx(0, 13) * TILE_H) << CSF), OBJ_IKACHAN);
			}
		}
		break;
	}
