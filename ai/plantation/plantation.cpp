...
void ai_midorin(Object *o)
{
	switch(o->state)
	{
		...
		case 3:		// running
		{
			...
			o->timer = random_nx(48, 64);		// how long to run
...
void ai_orangebell(Object *o)
{
	switch(o->state)
	{
		case 0:
		{
			...
			for(int i=0;i<8;i++)
			{
				Object *bat = CreateObject(o->CenterX() + random_nx(-0x1000, 0x1000), \
										   o->CenterY() + random_nx(-0x1000, 0x1000), \
										   OBJ_ORANGEBELL_BABY);
				bat->linkedobject = o;
			}
		}
...
void ai_orangebell_baby(Object *o)
{
	switch(o->state)
	{
		case 0:
		{
			o->xinertia = xinertia_from_angle(random_nx(0, 255), 0x200);
			o->yinertia = yinertia_from_angle(random_nx(0, 255), 0x200);
			o->flags |= FLAG_IGNORE_SOLID;
			
			o->timer = 0;							// time until can dive-bomb
			o->ymark2 = random_nx(-32<<CSF, 32<<CSF);	// unique target point on main bat
		...
		case 1:
		{
			...
			//DebugCrosshair(o->xmark, o->ymark, random_nx(0,255),random_nx(128,255),random_nx(128,255));
...
void ai_mimiga_farmer(Object *o)
{
	switch(o->state)
	{
		...
		case 1:
		{
			if (!random_nx(0, 60))
			{
				if (o->type != OBJ_MIMIGA_FARMER_STANDING && random_nx(0, 1))
				{	// walk
					o->state = 10;
				}
				else
