	...
	switch(o->state)
	{
		...
		case STATE_MEGA_BATS+1:
		{
			...
			if (o->timer > 20 && (o->timer % 3) == 1)
			{
				...
				bat->xinertia = random_nx(0x400, 0x800);
				bat->yinertia = random_nx(-0x200, 0x200);
				bat->dir = o->dir;
	...
	switch(o->state)
	{
		...
		case STATE_DISSOLVE+1:
		{
			...
			for(int i=0;i<3;i++)
			{
				...
				x = o->x + (random_nx(-16, 16) << CSF);
				y = (o->y - o->DrawPointY()) + (o->clipy1 << CSF);
				
				Object *drip = CreateObject(x, y, OBJ_RED_ENERGY);
				
				drip->xinertia = random_nx(-0x200, 0x200);
				drip->yinertia = random_nx(-0x400, 0);
				drip->angle = DOWN;
...
static void do_redsplode(Object *o)
{
	...
	for(int i=0;i<100;i++)
	{
		int x = o->x + (random_nx(-16, 16) << CSF);
		int y = o->y + (random_nx(-16, 16) << CSF);
		
		Object *spark = CreateObject(x, y, OBJ_RED_ENERGY);
		
		spark->xinertia = random_nx(-0x600, 0x600);
		spark->yinertia = random_nx(-0x600, 0x600);
		spark->angle = DOWN;
	}
}
