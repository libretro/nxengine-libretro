...
bool niku_load(uint32_t *value_out)
{
	...
	if (!fp)
	{
		fprintf(stderr, "niku_load: couldn't open file '%s'\n", fname);
	...
	if ((result[0] != result[1]) || \
			...
			(result[0] != result[3]))
	{
		fprintf(stderr, "niku_load: value mismatch: '%s', corrupt\n", fname);
		if (value_out) *value_out = 0;
	}
	else
	{
...
uint32_t *buf_dword = (uint32_t *)buf_byte;
	...
	// generate keys
	buf_byte[16] = random_nx(0, 255);
	buf_byte[17] = random_nx(0, 255);
	buf_byte[18] = random_nx(0, 255);
	buf_byte[19] = random_nx(0, 255);
