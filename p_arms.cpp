...
static void PFireMissile(int level, bool is_super)
{
	...
	if (level == 2)
	{
		...
		if (o->shot.dir==LEFT)
			o->xinertia = recoil_upper[is_super];
		else if(o->shot.dir==RIGHT)
			o->xinertia = -recoil_upper[is_super];
		else if (o->shot.dir==UP)
			o->yinertia = recoil_upper[is_super];
		else
			o->yinertia = -recoil_upper[is_super];
...
static void PHandleSpur(void)
{
	...
	else
	{
		if (spur->chargetimer)
		{
			if (spur->level > 0 && (!(CountObjectsOfType(OBJ_SPUR_SHOT))))
			{
