...
{
   // get the sprites used by the two objects
   SIFSprite *s1   = o1->Sprite();
   SIFSprite *s2   = o2->Sprite();

   // get the bounding rectangle of the first object
   int32_t rect1x1 = o1->x + (s1->solidbox.x1 << CSF);
   int32_t rect1x2 = o1->x + (s1->solidbox.x2 << CSF);
   int32_t rect1y1 = o1->y + (s1->solidbox.y1 << CSF);
   int32_t rect1y2 = o1->y + (s1->solidbox.y2 << CSF);

   // get the bounding rectangle of the second object
   int32_t rect2x1 = o2->x + (s2->solidbox.x1 << CSF);
   int32_t rect2x2 = o2->x + (s2->solidbox.x2 << CSF);
   int32_t rect2y1 = o2->y + (s2->solidbox.y1 << CSF);
   int32_t rect2y2 = o2->y + (s2->solidbox.y2 << CSF);
...
void Objects::RunAI(void)
{
   ...
   // because we handle objects in order of their 
   // creation and have a separate list
   // for display order, we can't ever run AI twice 
   // in a frame because of z-order
   // rearrangement, and 2) objects created by other 
   // objects are added to the end of
   // the list and given a chance to run their 
   // AI routine before being displayed.
   FOREACH_OBJECT(o)
   {
      if (!o->deleted)
         o->RunAI();
