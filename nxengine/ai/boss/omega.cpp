	...
	switch(o->state)
	{
		...
		case OMG_FIRE:	// throwing out red stuff
		{
			...
			if (omg.firecounter > omg.startfiring && omg.firecounter < omg.stopfiring)
			{
				if ((omg.firecounter % omg.firefreq)==0)
				{
					...
					shot->xinertia = nx_random(-omg.shotxspd, omg.shotxspd);
					shot->yinertia = -0x333;
					if (omg.form==2 || nx_random(0, 9) < 8)
					{
					...
					shot->timer = (nx_random(0, 7) >= 4) ? nx_random(300, 400):0;
					shot->damage = 4;
				}
			}
		...
		case OMG_UNDERGROUND:		// underground waiting to reappear
		{
			if (++omg.timer >= 120)
			{
				...
				o->x = omg.orgx + (nx_random(-64, 64) << CSF);
				o->y = omg.orgy;
				omg.movetime = OMEGA_RISE_HEIGHT;
		...
		case OMG_EXPLODING+1:
		{
			...
			x = o->CenterX() + (nx_random(-48, 48)<<CSF);
			y = o->CenterY() + (nx_random(-48, 24)<<CSF);
			SmokePuff(x, y);
