		...
		case CR_FaceDoom+1:
		{
			...
			if (o->timer3 == 75)
			{
				CreateObject(MAPX(map.xsize) + 40, \
							 MAPY(3 + nx_random(-3, 0)), OBJ_UDMINI_PLATFORM);
			}
			...
			if (o->timer3 == 150)
			{
				o->timer3 = 0;
				CreateObject(MAPX(map.xsize) + 40, \
							 MAPY(10 + nx_random(-1, 3)), OBJ_UDMINI_PLATFORM);
...
void UDCoreBoss::SpawnFaceSmoke()
{
	...
	for(int i=0;i<8;i++)
	{
		int x = face->x + nx_random(-16<<CSF, 32<<CSF);
		int y = main->CenterY();
		Object *s = SmokePuff(x, y);
		s->xinertia = nx_random(-0x200, 0x200);
		s->yinertia = nx_random(-0x100, 0x100);
	}
}
...
bool UDCoreBoss::RunDefeated()
{
	...
	switch(o->state)
	{
		...
		case CR_Exploding:
		{
			...
			int x = o->x + nx_random(-72<<CSF, 72<<CSF);
			int y = o->y + nx_random(-64<<CSF, 64<<CSF);
			SmokePuff(x, y);
...
void ai_udmini_platform(Object *o)
{
	switch(o->state)
	{
		case 0:
		{
			...
			o->xinertia = -0x200;
			o->yinertia = 0x100;
			if (nx_random(0, 1)) o->yinertia = -o->yinertia;
		}
		case 1:
		{
			if (o->x < -(64 << CSF))
...
void ai_ud_blast(Object *o)
{
	...
	SmokePuff(o->CenterX() + (nx_random(0, 16) << CSF), \
			  o->CenterY() + (nx_random(-16, 16) << CSF));
