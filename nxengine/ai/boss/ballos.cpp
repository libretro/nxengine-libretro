...
void BallosBoss::RunForm3(Object *o)
{
	...
	switch(o->state)
	{
		...
		case CS_SPIN_PLATFORMS+1:
		{
			...
			switch(++o->timer)
			{
				...
				case 300:	// spawn archers on side
				{
					...
					for(int i=0;i<8;i++)
					{
						...
						int x = (nx_random(-TILE_W, TILE_W) & ~3) << CSF;
						int y = (nx_random(2 * TILE_H, 17 * TILE_H) & ~3) << CSF;
			...
			if (!nx_random(0, prob))
			{
				CreateObject(o->x + nx_random(-40<<CSF, 40<<CSF), \
							 o->y + nx_random(0, 40<<CSF),
...
void BallosBoss::RunDefeated(Object *o)
{
	switch(o->state)
	{
		...
		case 1001:
		{
			int x = o->x + nx_random(-60<<CSF, 60<<CSF);
			int y = o->y + nx_random(-60<<CSF, 60<<CSF);
			SmokePuff(x, y);
