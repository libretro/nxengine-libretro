...
{
   int i;
   float_type curenv = env->initial;
   float_type envinc = (float_type)(env->val[0] - env->initial) / env->time[0];
...
static void CreateAudio(stPXChannel *chan)
{
   ...
   for(i=0;i<size_blocks;i++)
   {
      ...
      if (pitch->model_no==MOD_PULSE)
         bm = (signed char)bm;
      if (pitch2->model_no==MOD_PULSE)
         bm2 = (signed char)bm2;
...
void pxt_PrepareToPlay(stPXSound *snd, int slot)
{
   ...
   int malc_size           = (snd->final_size * 2 * 2);
   signed short *outbuffer = (signed short *)malloc(malc_size);
...
static void pxtLooper(int chan, int slot)
{
	...
	if (sound_fx[slot].loops_left > 0)
      sound_fx[slot].loops_left--;
}
...
char pxt_LoadSoundFX(RFILE *fp, int top)
{
   ...
   for(slot=1;slot<=top;slot++)
   {
      ...
      // upscale the sound to 16-bit 
      // for SDL_mixer then throw away 
      // the now unnecessary 8-bit data
