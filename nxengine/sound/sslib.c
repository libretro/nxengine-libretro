...
static int AddBuffer(struct SSChannel *chan, int bytes)
{
   ...
   if (chunk->bytepos+bytes > chunk->bytelength)
   {
      ...
      // only add what's left. and advance the 
      // head pointer to the next chunk.
...
// returned by SSFindFreeChannel and no
// sounds will be allocated to it by SSEnqueueChunk with a -1 parameter.
// thus, the channel can only be played on by 
// explicitly playing a sound to it.
